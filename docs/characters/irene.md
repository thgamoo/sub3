# 성녀후보 - 이레네 (Irene)

## 기본 정보

- 구분: 서브 빌런 (플레이어 캐릭터)
- 시작지점: 중부
- 시작자원: 신성 2, 매력 1
- 모티프: 이레네 (Irene)

## 외모

- 앞머리로 눈을 가림
- 연노랑 머리색
- 금안
- 머리에 사제관을 쓰고 있음
  - 주교관과 비슷하지만, 사제관은 신성력으로 사람들을 치유하고 악마를 처단하는 이들을 나타내는 증표

## 원래 운명

그녀는 어렸을 적 악마숭배자들에게 마을을 공격받았고 교회에게 구원받았다. 수녀원에서 신성력을 각성했고 교회는 이를 달갑게 여긴다. 교회는 루크레치아의 전담 성직자로 그녀를 파견하며, 그녀의 부탁을 가능한 한 들어주라는 명령을 내린다. 그녀는 그 역할을 충실히 이행하다 황태자의 동생의 두 눈을 뽑는 악행을 저지르게 되는데, 이에 분노한 주인공 일행에게 죽음을 맞이하게 된다.

## 가능한 각성 능력

- 일반 각성 능력 전부

## 숨겨진 과거

그녀는 사실 메살리나 - 루크레치아 거래의 희생자다. 그녀가 머물던 마을은 제노비아가 머물던 동남부의 한적한 마을 옆 마을이었고, 흥분한 악마숭배자들에 의해 표적이 된 것이다. 그녀의 아버지는 신성교회의 추기경이었으며 시골의 아낙네와 결혼해 파문을 당했다. 어떤 사실도 모른 채 이레네는 다시 교회로 돌아간 것이다.

## 과거의 단서

- "이교의 계약서"
- "교황칙서 (말살 - HAL, ZEN 이 적혀있음)"
- "폐태자의 편지"
- "추기경서임서"

## 이미지 생성 프롬프트 v10

> **기도 포즈 버전.**
> 두 손을 가슴 앞에서 **손바닥끼리 맞댄 정식 기도 자세**로 못박았다 — `pressed together palm to palm`, `fingers straight and pointing upward`, `elbows drawn in close`. 손 모양이 흐트러지지 않도록 네거티브에 `(hands apart:1.5) (one hand:1.5) hands at sides, reaching out` 을 걸었다.
>
> ⚠️ **성스럽게만 보이면 이 캐릭터가 아니다.** 앞머리가 눈을 완전히 덮고 금빛만 새어나오는 기존 설정을 유지하고, 표정을 `completely blank expressionless` 로 둔다. 후광도 `very faint pale halo` 로 약하게 — 네거티브에 `(warm golden glow:1.3) strong halo, blinding light, divine radiance` 를 넣어 경건함이 과해지는 것을 막았다. **겉은 독실하고 속은 안 읽히는 상태**가 목표다.
>
> `fate_art.md` 의 `irene_heretic` v7 이 붉은 낙인으로 규탄을 그린다면, 이쪽은 그 반대쪽 얼굴이다.

```text
cowboy shot, front view, eye level,

Irene, a serene yet unsettling saintess candidate, standing in prayer,

both hands raised and pressed together palm to palm in front of her chest in a formal prayer gesture, fingers straight and pointing upward, elbows drawn in close to her body, her head bowed very slightly, her chin lowered, a completely blank expressionless face, lips closed and relaxed, no smile, pale blonde hair with long heavy straight bangs falling completely over her eyes, only a faint golden glow seeping through the hair where her eyes would be, white and gold priestess vestments with a tall pointed white mitre, a golden cross embroidered down the front of her robe, wide sleeves falling away from her raised forearms, a very faint pale halo of light behind her head, quiet and still, devout on the surface and unreadable underneath, (flat color:1.3), (anime coloring:1.3), bold clean outlines, hard-edged cel shading, matte finish, muted dark neutral gradient background with a few drifting light particles, vertical composition
```

## 네거티브 프롬프트 v10

```text
(hands apart:1.5), (one hand:1.5), hands at sides, arms down, reaching out, outstretched arm, open palm facing the viewer, waving, pointing, holding an object, book, staff, rod, candle, rosary, beads, (visible eyes:1.4), uncovered eyes, bangs parted, eyes showing through, smiling, gentle smile, grin, laughing, crying, tears, angry, kneeling, sitting, lying down, bowing deeply, head fully down, (warm golden glow:1.3), strong halo, blinding light, divine radiance, light beam from above, soft airbrush shading, smooth gradient shading, subsurface scattering, glossy skin, shiny skin, specular highlight, plump lips, glossy lips, sparkling eyes, catchlight, 3d, realistic, photorealistic, rendered, cgi, second person, two people, crowd, text, watermark, signature, logo, extra fingers, deformed hands, six fingers, fused fingers, bad anatomy, lowres, blurry
```

## 이미지 생성 프롬프트

```text
A serene yet unsettling saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long bangs covering her eyes, faint glimpse of golden glowing eyes through the bangs, gentle mysterious smile, extremely ornate white and gold priestess vestments, elaborate jeweled mitre-like veil and priest crown symbolizing holy healing and demon-slaying power, softly glowing circular halo of divine light and floating holy scripture pages, grand cathedral interior with bright stained-glass light in the background, holy but eerie atmosphere, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v2

> 수정: 성녀처럼 과하게 나오는 문제. 옷 장식 제거, 왕관 대신 주교관, 후광·성서 페이지 제거.

```text
A serene yet unsettling saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long bangs covering her eyes, faint glimpse of golden glowing eyes through the bangs, gentle mysterious smile, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet cathedral interior with soft light in the background, holy but subtly eerie atmosphere, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v3

> 수정: v2는 눈이 훤히 드러나고 미소가 뚜렷해서 **서브 빌런 느낌이 약했음**.
> 앞머리로 눈을 **완전히** 가리고, 미소 대신 **무표정**으로. 금안은 앞머리 사이로
> 희미하게 비치는 정도만. 감정 없이 악행을 저지르는 공허함을 노린다.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs falling completely over her eyes, her eyes fully hidden behind her hair, only a faint golden glow seeping dimly through the strands, blank expressionless face with lips set in a flat neutral line, no smile at all, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, the emptiness of someone who commits atrocities without feeling anything, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v4

> 수정: v3에서 눈이 완전히 가려지면서 **금안이 화면에서 사라짐**.
> 앞머리로 가리는 것과 무표정은 그대로 두고, **머리카락 사이로 금안이 또렷이 비치도록** 강화.
> v3 네거티브의 `visible eyes, eyes showing` 이 앞머리 사이 미광까지 죽였던 것이 원인.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs falling over her eyes, her eyes never uncovered yet her glowing golden eyes clearly visible shining through the gaps between the strands of her bangs, the golden glow bright and distinct behind the hair, blank expressionless face with lips set in a flat neutral line, no smile at all, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, the emptiness of someone who commits atrocities without feeling anything, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v5

> 수정: v4는 금안이 살아났지만 **눈이 머리카락 "위에" 그려져** 사실상 노출된 상태가 됨.
> **앞머리가 눈을 완전히 덮는 것이 이 캐릭터의 핵심 포인트.**
> 머리카락 가닥이 눈 앞을 확실히 지나가게 하고, 금안은 그 **뒤에서 새어나오는 빛**으로만
> 존재를 알린다 — 눈 모양이 또렷이 그려지는 것이 아니라 어렴풋이 짐작되는 정도.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs hanging completely over both eyes, individual strands of hair clearly passing in front of her eyes and never parted, her eyes entirely behind the curtain of hair, only a golden glow bleeding through the narrow gaps between the strands so the shape of her eyes is dimly suggested rather than clearly drawn, blank expressionless face with lips set in a flat neutral line, no smile at all, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v8

> **v7에서 사제관 형태만 교정.**
> v7의 모자가 납작하고 둥근 캡처럼 나왔음. 사제관은 **위로 솟아 뾰족한 형태**여야 함
> (앞뒤 두 면이 위에서 모여 꼭짓점을 이루는 전형적인 주교관 실루엣).
> 나머지(앞머리로 눈 완전히 덮기 · 무표정 · 정면 · 흰 제의)는 v7 그대로.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs falling completely over her eyes, her eyes fully hidden behind her hair, only a faint golden glow seeping dimly through the strands, blank expressionless face with lips set in a flat neutral line, no smile at all, facing forward with her head level, simple unadorned white priestess vestments with only minimal gold trim, wearing a tall pointed bishop's mitre rising high above her head and narrowing to a sharp peak at the top, the classic two-panel mitre silhouette with a clearly pointed apex, white with slim gold trim and a small gold cross on the front, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, the emptiness of someone who commits atrocities without feeling anything, elegant half body portrait, high detail, no text, no watermark
```

## 이미지 생성 프롬프트 v9

> 구도: **올려다보기.** 고개가 젖혀지면 사제관 실루엣이 살고 앞머리가 뜬다. (금안이 비치는 난제는 해결하지 않아도 된다는 판단)

```text
looking up, head tilted back, cowboy shot, eye level,

Irene, a quietly ominous saintess candidate with her face turned upward toward the light,

 pale yellow blonde hair with long heavy straight bangs falling completely over her eyes, her eyes fully hidden behind her hair, only a faint golden glow seeping dimly through the strands, blank expressionless face with lips set in a flat neutral line, no smile at all, simple unadorned white priestess vestments with only minimal gold trim, wearing a tall pointed bishop's mitre rising high above her head and narrowing to a sharp peak at the top, the classic two-panel mitre silhouette with a clearly pointed apex, white with slim gold trim and a small gold cross on the front, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, the emptiness of someone who commits atrocities without feeling anything, vertical composition
```

## 네거티브 프롬프트 v9

```text
head level, facing forward, looking at viewer, looking down,
flat topped hat, round hat, pillbox hat, soft cap, low hat, short hat, brimmed hat, nurse cap, beanie, hood, veil only, closed eyes, downcast face, looking down, head tilted down, light beam, light shaft, glowing pillar of light, lens flare, smiling, smile, grin, gentle smile, happy expression, cheerful, kind expression, visible eyes, eyes showing, eyes above the bangs, parted bangs, short bangs, hair tucked behind ears, crown, halo, floating scripture pages, ornate vestments, heavy jewelry, dark hair, red hair, text, watermark, signature, logo, extra fingers, deformed hands, bad anatomy, lowres, blurry
```

## 네거티브 프롬프트 v8

```text
flat topped hat, round hat, pillbox hat, soft cap, low hat, short hat, brimmed hat, nurse cap, beanie, hood, veil only, closed eyes, downcast face, looking down, head tilted down, light beam, light shaft, glowing pillar of light, lens flare, smiling, smile, grin, gentle smile, happy expression, cheerful, kind expression, visible eyes, eyes showing, eyes above the bangs, parted bangs, short bangs, hair tucked behind ears, crown, halo, floating scripture pages, ornate vestments, heavy jewelry, dark hair, red hair, text, watermark, signature, logo, extra fingers, deformed hands, bad anatomy, lowres, blurry
```

> v8 네거티브 요점: v7 네거티브 + 모자 형태 교정용
> `flat topped hat, round hat, pillbox hat, soft cap, low hat, short hat, brimmed hat,
> nurse cap, beanie, hood` 추가. 나머지는 v7에서 유지.

## 이미지 생성 프롬프트 v7

> **최종안. v3 복원 + 안전장치.**
>
> 결론: **머리카락 사이로 비치는 금안은 이 모델에서 안정적으로 안 나온다.**
> 시도 이력 —
> - v3: 눈 완전히 가림 ✓ / 금안 사라짐 ✗
> - v4: 금안 살아남 ✓ / 눈이 머리카락 **위에** 그려져 사실상 노출 ✗
> - v5: 눈 완전히 가림 ✓ / 금안 다시 사라짐 ✗, 게다가 고개 숙이고 눈 감는 컷 발생
> - v6: 발광 가중치 1.4~1.5 → **화면을 관통하는 빛기둥 아티팩트** 발생 ✗
>
> → **금안은 포기하고 "앞머리가 눈을 완전히 덮는다"는 핵심만 취한다.**
> v3 프롬프트를 그대로 쓰되 v5에서 나온 고개 숙임·눈 감음만 네거티브로 차단.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs falling completely over her eyes, her eyes fully hidden behind her hair, only a faint golden glow seeping dimly through the strands, blank expressionless face with lips set in a flat neutral line, no smile at all, facing forward with her head level, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, the emptiness of someone who commits atrocities without feeling anything, elegant half body portrait, high detail, no text, no watermark
```

## 네거티브 프롬프트 v7

```text
closed eyes, downcast face, looking down, head tilted down, light beam, light shaft, glowing pillar of light, lens flare, smiling, smile, grin, gentle smile, happy expression, cheerful, kind expression, visible eyes, eyes showing, eyes above the bangs, parted bangs, short bangs, hair tucked behind ears, crown, halo, floating scripture pages, ornate vestments, heavy jewelry, dark hair, red hair, text, watermark, signature, logo, extra fingers, deformed hands, bad anatomy, lowres, blurry
```

> v7 네거티브 요점: v3 네거티브 + `closed eyes, downcast face, looking down, head tilted down`
> (v5의 고개 숙임·눈 감음 방지) + `light beam, light shaft, glowing pillar of light, lens flare`
> (v6의 빛기둥 아티팩트 방지).

## 이미지 생성 프롬프트 v6

> 수정: v5가 "앞머리로 눈 완전히 덮기"는 달성했으나 **금안이 다시 사라짐**.
> v5 네거티브의 `clearly outlined eyes, sharp eye outline, visible pupils, visible eyelashes` 가
> 눈 렌더링 자체를 죽이면서 발광까지 같이 없앴음 (v3와 같은 실수).
>
> **억제는 레이어 순서에만 걸고**(눈이 머리카락 위로 올라오는 것), 발광은 **가중치로** 민다.
> 모델 카드: 프롬프트 가중치는 SDXL보다 높게 줘야 먹힘.

```text
A quietly ominous saintess candidate, romance fantasy webnovel style, detailed anime character portrait, pale yellow blonde hair with long heavy straight bangs hanging completely over both eyes, individual strands of hair passing in front of her eyes and never parted, her eyes entirely behind the curtain of hair, (a bright golden light glowing out through the narrow gaps between her bangs:1.5), (the golden glow of her hidden eyes clearly shining through the hair:1.4), the eye shape only dimly suggested behind the strands, blank expressionless face with lips set in a flat neutral line, no smile at all, simple unadorned white priestess vestments with only minimal gold trim, plain white and gold bishop's mitre on her head, no crown, no halo, no floating scripture pages, no jewelry, quiet dim cathedral interior with soft light in the background, holy but hollow and unsettling atmosphere, elegant half body portrait, high detail, no text, no watermark
```

## 네거티브 프롬프트 (Negative Prompt)

```text
eyes drawn in front of hair, eyes over the bangs, hair behind the eyes, eyes fully exposed, bangs pushed aside, bangs parted, short bangs, hair tucked behind ears, hair clipped back, closed eyes, downcast face, smiling, smile, grin, gentle smile, happy expression, cheerful, kind expression, crown, halo, floating scripture pages, ornate vestments, heavy jewelry, dark hair, red hair, text, watermark, signature, logo, extra fingers, deformed hands, bad anatomy, lowres, blurry
```

> v6 네거티브 요점: **레이어 순서 교정 항목만 유지**하고
> (`eyes drawn in front of hair, eyes over the bangs, hair behind the eyes`),
> 눈 렌더링을 죽이는 항목(`clearly outlined eyes, visible pupils, visible eyelashes` 등)은 **전부 제거**.
> v5에서 얼굴을 숙여 눈을 감는 컷이 나왔으므로 `closed eyes, downcast face` 추가.
>
> ⚠️ **이 캐릭터의 반복 실패 패턴**: 눈을 가리려고 네거티브를 조이면 금안이 사라지고,
> 금안을 살리려고 풀면 눈이 머리카락 위로 올라온다. 둘의 균형점이 좁으므로
> 네거티브에 **눈 자체를 지우는 표현을 넣지 말 것**.

> v5 네거티브 요점: **레이어 순서를 교정**하는 항목이 핵심 —
> `eyes drawn in front of hair, eyes over the bangs, hair behind the eyes` 로
> 눈이 머리카락 위에 올라오는 것을 막고, `clearly outlined eyes, sharp eye outline,
> visible pupils, visible eyelashes` 로 눈 모양이 또렷해지는 것을 억제한다.
> 단 발광 자체를 막는 표현(`glowing eyes` 류)은 절대 넣지 말 것 — v3에서 금안이 사라진 원인.

> v4에 맞춰 네거티브 조정: `visible eyes, eyes showing` **제거** — 이 둘이 앞머리 사이로
> 비치는 금안까지 같이 억제했음. 대신 `eyes fully exposed, bangs pushed aside, bangs parted,
> hair clipped back` 로 **눈이 드러나는 것만** 막는다. 미소 억제 항목은 v3에서 유지.

> v3에 맞춰 네거티브 강화: 미소 억제(`smiling, smile, grin, gentle smile, happy expression,
> cheerful, kind expression`)와 눈 노출 억제(`visible eyes, eyes showing, eyes above the bangs,
> parted bangs, short bangs, hair tucked behind ears`) 추가.
> 기존의 `fully visible eyes` 는 위 항목들로 대체됨.
